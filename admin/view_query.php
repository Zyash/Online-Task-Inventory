...
}else
{
	...
	<title>WORKFLOW | Query Management</title>
  ...
  <div id="containerHolder">
    <div id="container">
      ...
      <!-- h2 stays for breadcrumbs -->
        <h2>Query Management &raquo; <a href="#" class="active"> Pending Query</a></h2><br/>  
        ...
        <div id="main">
            ...
            <?php 
              	$obj->query="select * from query_details where status='pending'";
				//echo $obj->query;
				$obj->select($obj->query);
				...
				?>
						...
						<fieldset>
						<legend><?php echo strtoupper($row["query_type"]);?></legend>
						<li><h3><?php echo $row["emp_name"];?></h3></li>
                        <ul>
							<!-- <li class="pull-right"><?php //echo $days."day(s) ago";?></li -->
                            <li>Query Type:<?php echo $row["query_type"]?></li>
                            <li>START DATE:<?php echo $row["start_date"]?></li>
                            <li>Reason:<?php echo $row["reason"]?></li>
							<li>
                            <form name="frm" method="post" action="action/query-status.php">
                            <input type="hidden" name="hdnl_id" value="<?php echo $row["q_id"]?>" />
